package cn.guankejian.simplewindow
...
import cn.guankejian.util.logE


class  SimpleWindow  constructor(context: Context) :
  ScreenOrientationMonitor.OnScreenOrientationCallback {
   companion object{
    private val instances:MutableList<SimpleWindow> = ArrayList()
    private val HANDLER: Handler = Handler(Looper.getMainLooper())
    @JvmStatic
    fun with(application: Application): SimpleWindow {
      return SimpleWindow(application)
    }
  }
  ...
  private var exitAnim:Int = R.anim.left_to_right
  private var enterAnim:Int = R.anim.right_to_left
  ...
  fun setContentView(id: Int): SimpleWindow {
    return setContentView(LayoutInflater.from(_context).inflate(id, decorView, false))
  }

  fun setExitAnim(id:Int): SimpleWindow {
    this.exitAnim = id
    return this
  }

  fun setEnterAnim(id:Int): SimpleWindow {
    this.enterAnim = id
    return this
  }


  fun setXOffset(px: Int): SimpleWindow {
    windowParams.x = px
    postUpdate()
    post {
   ...
   * 设置垂直偏移量
   */
  fun setYOffset(px: Int): SimpleWindow {
    windowParams.y = px
    postUpdate()
    post {
        _draggable?.refreshLocationCoordinate()
  ...
  fun setDraggable(draggable: BaseDraggable): SimpleWindow {
    ...
    if (mScreenOrientationMonitor == null) {
      mScreenOrientationMonitor =
        ScreenOrientationMonitor(
          _context.getResources().getConfiguration()
        )
    }
   ...
   * 移除窗口标记
   */
  fun removeWindowFlags(flags: Int): SimpleWindow {
    windowParams.flags = windowParams.flags and flags.inv()
    postUpdate()
    return this
  }
  fun setWidth(width: Int): SimpleWindow {
   ...
   * 设置悬浮窗高度
   */
  fun setHeight(height: Int): SimpleWindow {
   ...
   * 设置悬浮窗显示的重心
   */
  fun setGravity(gravity: Int): SimpleWindow {
  ...
  fun setContentView(view: View): SimpleWindow {
    if (decorView.childCount > 0) {
  ...
  private fun setWindowType(type: Int): SimpleWindow {
  ...
  fun resume(){
    ...
    decorView[0].startAnimation(AnimationUtils.loadAnimation(_context, enterAnim))
  }
  fun pause(){
    if(!showing)
      return
    showing = false
    decorView[0].startAnimation(AnimationUtils.loadAnimation(_context, exitAnim).also{
  ...
  interface OnWindowLifecycle {
     ...
     * 显示回调
     */
    fun onWindowShow(window: SimpleWindow) {}

    /**
     * 消失回调
     */
    fun onWindowCancel(window: SimpleWindow) {}

    /**
     * 回收回调
     */
    fun onWindowRecycle(window: SimpleWindow) {}
  }
